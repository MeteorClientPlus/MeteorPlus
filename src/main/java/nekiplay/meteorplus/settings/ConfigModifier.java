			...
			if (spoofMode.get() == SpoofMode.Fake) {
				ss.set("camera", new ValueMap()
					.set("pos", new ValueMap()
						...
						.set("x", () -> Value.number(mc.gameRenderer.mainCamera().position().x + ConfigModifier.get().x_spoof.get()))
						.set("y", () -> Value.number(mc.gameRenderer.mainCamera().position().y))
						.set("z", () -> Value.number(mc.gameRenderer.mainCamera().position().z + ConfigModifier.get().z_spoof.get()))
					)
